...
var load = require('pug-load-cache');
...
function compileBody(str, options) {
  ...
  var ast = load.string(str, {
    ...
    loadFromStrArr: options.loadFromStrArr,
    ...
    read: function (filename, loadOptions) {
      let content = loadOptions.loadFromStrArr.filter(e => {
        let name = filename
        if (plateform == "win32" || plateform == "win64") {
          name = filename.replace(/\\/, "").replace(/\\/, "/")
        } else {
          name = filename.replace(/\//, "")
        }
        return e.name.trim() === name
      })[0]
      if (content && content.name && content.content) filename = content.name
...
function getDependencies(str, options) {
  var debug_sources = {};
  debug_sources[options.filename] = str;
  var dependencies = [];
  var plugins = options.plugins || [];
...
function handleTemplateCache(options, str) {
  ...
  if (options.cache && exports.cache[key]) {
    ...
    if (str === undefined) {
      ...
      let content = options.loadFromStrArr.filter(e => {
        if (plateform == "win32" || plateform == "win64") {
...
/**
 * let fn = new Function("pug",funStr)(require("pug-runtime"))
 * let html = fn(data)
 * @return {funStr:String,dependencies:[]}
 */
exports.compileToFunStr = function (str, options) {
  var options = options || {}
  ...
  return { funStr: parsed.body + ";return template;", dependencies: parsed.dependencies }
};
...
exports.compileFile = function (path, options) {
  ...
  options.loadFromStrArr = options.loadFromStrArr || []
...
exports.render = function (str, options, fn) {
  ...
  return handleTemplateCache(options, str)(options.userData);
};
